...
print("=" * 60)
print(f"PBID.JK DATA FETCH — {datetime.utcnow().isoformat()}Z")
print("=" * 60)
...
try:
    ...
    block_a = {
        ...
        "daily_bars_1y":    {k: v for k, v in list(hist_1y[["Open","High","Low","Close","Volume"]].to_dict("index").items())[-10:]},  # only print last 10 for log brevity
    ...
    if price_data["current_price"] is None and len(hist_1y) < 10:
        print("❌ BLOCK A ABORT: No price and insufficient bars.")
        sys.exit(1)
...
try:
    ...
    for item in raw_news[:5]:
        ...
        publisher_info = content.get("provider", {}) if content else {}
        publisher = publisher_info.get("displayName") if publisher_info else item.get("publisher")
    ...
    print(f"❌ BLOCK D FAILED: {e}")
    news_output = {"headline_count": 0, "headlines": []}
